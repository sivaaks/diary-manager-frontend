...
export default function Register(){
    ...
    const [showAlert,setShowAlert]=useState({show:false,vertical:'top',horizontal:'right',type:'success',message:''});
    ...
    const registerUser=async()=>{
            ...
            const data=await axios.post(API_REGISTER,
                ...
                if(res.data) if(res.status===200) {
                    setLoading(false); 
                    setShowAlert({...showAlert,type:'success',message:'Registration completed successfully',show:true});
    ...
    return(
        <>
         <Container maxWidth="sm">
            ...
            <Link onClick={()=>history.push('/forgot-password')} variant="body2" color="primary" sx={{cursor:'pointer'}}>Forgot password?</Link>
            </Stack>
        </Container>
        <Snackbar open={show} autoHideDuration={6000} onClose={alertBoxClose} anchorOrigin={{vertical,horizontal}}>
